            ...
            <div>
                {EXPERIENCES.map((experience, index) => (
                    <div key={index} className='mb-8 flex flex-wrap lg:justify-center'>
                        ...
                        <p className='mb-4 text-slate-400 '>{experience.description}</p>
                        {experience.technologies.map((tech, index)=>(
                            <span className='mr-2 mt-4 rounded bg-stone-900  px-2 py-1 text-[10px] font-medium text-stone-300 'key={index}>
                                {tech}
                            </span>
                        ))}
                        </motion.div>
